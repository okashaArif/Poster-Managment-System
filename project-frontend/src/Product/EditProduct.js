...
export default function EditProduct() {
    ...
    const onSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`http://localhost:8084/products/${id}`, product);
            navigate('/allproducts'); // Redirect after successful update
        } catch (error) {
            ...
            <div className="row">
                <div className="col-md-6 offset-md-3 border rounded p-4 mt-2 shadow">
                    ...
                    <form onSubmit={onSubmit}>
                        ...
                        <button type='button' className='btn btn-outline-danger mx-2' onClick={() => navigate('/allproducts')}>Cancel</button>
                    </form>
                </div>
            </div>
        </div>
